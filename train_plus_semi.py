...
from loss import (
    ...
    Multi_Cross_entropy,
    Bidirection_consistency_loss_semi,
    dice_score,
    get_non_zero_target_v3,
    mask2onehot,
)
from OCTdataset.dataset import UniversalOCTTrainDataset_Semi, UniversalOCTDataset
from OCTdataset.sampler import TwoStreamSampler_semi_DDP
...
def build_parser():
    ...
    parser.add_argument('--result_path', type=str, default='/data/zhongj/Codes/UniOCTPlus_result/output_')
    parser.add_argument('--exp', type=str, default='new_semi_20per_0313',
    ...
    parser.add_argument('--device', type=int, default=7,
                        help='Fallback single-device id.')
...
def build_dataloaders(args):
    train_dataset = UniversalOCTTrainDataset_Semi(
        root=os.path.join(args.data_pth, 'new_train_data_0823_20per.txt'),
        train_test_flag='train'
    )

    train_sampler = TwoStreamSampler_semi_DDP(
        train_dataset,
    ...
    val_dataset = UniversalOCTDataset(
        root_path=os.path.join(args.data_pth, 'new_val_data.txt'),
        split='val'
    )
    ...
    return train_dataset, train_sampler, train_loader, val_dataset, val_loader
...
def load_checkpoint_if_needed(args, model, ema_model, optimizer, ema_helper):
    ...
    if 'ema_model' in checkpoint:
        ...
        best_performance = checkpoint.get('layer_performance', 0.0)
    else:
...
def train(args, snapshot_path):
    ...
    _, train_sampler, train_loader, _, val_loader = build_dataloaders(args)
    ...
    loss_consistency = Bidirection_consistency_loss_semi(logger)
    ...
    for epoch in iterator:
        ...
        for idx, (imgs, labels, task_onehots, dataset_names) in enumerate(train_loader):
            ...
            layer_loss_dice = loss_dice(pred_logits[labeled_idx:], labels[labeled_idx:])
            layer_loss_ce = loss_ce(pred_logits[labeled_idx:], labels[labeled_idx:])
            ...
            if iter_num > 1000:
                ...
                consistency_loss = loss_consistency(
                    ema_probs, probs, task_onehots, labeled_bs=labeled_idx
                )
            ...
            if iter_num >= 20000 and iter_num % 1000 == 0:
                ...
                if best_performance <= ema_dice and dist.get_rank() == 0:
