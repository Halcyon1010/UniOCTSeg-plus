...
# =============================================================================
#  1. Activation Functions & Helpers
# =============================================================================
...
def _gelu_python(x):
    """Original Python implementation of GELU."""
    return x * 0.5 * (1.0 + torch.erf(x / math.sqrt(2.0)))

# Use PyTorch's native GELU if available, else fallback
gelu = getattr(F, "gelu", _gelu_python)

def gelu_new(x):
    """
    Gaussian Error Linear Unit (GELU) approximation.
    Often used in BERT and ViT models.
    """
...
def get_activation(activation_string):
    if activation_string in ACT2FN:
        ...
        raise KeyError(f"Function {activation_string} not found in ACT2FN mapping.")

# Helper classes for the Decoder (SelfAttentionLayer, CrossAttentionLayer, FFNLayer) 
# have been consolidated to standard implementations for brevity, 
# ensuring they follow the Pre/Post norm logic defined in the original code.
...
class SelfAttentionLayer(nn.Module):
    ...
    def forward(self, tgt, tgt_mask=None, query_pos=None, **kwargs):
        tgt2 = self.norm(tgt) if self.normalize_before else tgt
        q = k = self.with_pos_embed(tgt2, query_pos)
        tgt2 = self.self_attn(q, k, value=tgt2, attn_mask=tgt_mask)[0]
        tgt = tgt + self.dropout(tgt2)
        return tgt if self.normalize_before else self.norm(tgt)
...
class CrossAttentionLayer(nn.Module):
    ...
    def forward(self, tgt, memory, memory_mask=None, pos=None, query_pos=None, **kwargs):
        tgt2 = self.norm(tgt) if self.normalize_before else tgt
...
class Encoder(nn.Module):
    def __init__(self, hidden_size, num_heads, vis, layer_num=12):
        super(Encoder, self).__init__()
        self.vis = vis
        self.layer = nn.ModuleList()
        self.encoder_norm = LayerNorm(hidden_size, eps=1e-6)
        for _ in range(layer_num):
            layer = Block(hidden_size, num_heads, vis)
...
class ViT(nn.Module):
    def __init__(self, hidden_size=768, num_heads=12, layer_num=12):
        super(ViT, self).__init__()
        self.transformer = Encoder(hidden_size=hidden_size, num_heads=num_heads, vis=False, layer_num=layer_num)
